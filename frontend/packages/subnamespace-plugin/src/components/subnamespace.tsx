...
const SubnamespaceTableRow: RowFunction<K8sResourceKind> = ({ obj, index, key, style }) => {
...
const ClusterResourceQuotaTab: React.FC<SubnamespaceDetailsProps> = ({ obj }) => {
  const {
    metadata: { name },
  } = obj;
  // var filter = 'dana.hns.io/aggregator-' + name;
  return (
    <ClusterResourceQuotasPage
      selector={{
        'crq.subnamespace': name,
        // 'dana.hns.io/aggregator-' + name : 'true'
      }}
    />
  );
};
...
const mainCards = [{ Card: DetailsCard }, { Card: UtilizationCard }];
// const leftCards = [{ Card: DetailsCard }, { Card: StatusCard }];
const rightCards = [{ Card: StatusCard }, { Card: ResourceQuotaCard }];

const SnsDashboard: React.FC<SubnamespaceDetailsProps> = ({ obj }) => {
  const context = {
    obj,
  };
  const mainGridCards = React.useMemo(() => mapCardsToGrid(mainCards, 'main'), []);
  // const leftGridCards = React.useMemo(() => mapCardsToGrid(leftCards, 'left'), [
  //   leftCards,
  // ]);
...
const pages = [
  ...
  navFactory.editYaml(),
  {
    href: 'clusterresourcequotas',
    name: 'Quota',
    component: ClusterResourceQuotaTab,
  },
  {
    //  href: 'updatequota',
    //  name: 'Update Quota',
